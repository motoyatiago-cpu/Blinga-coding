...
test("adds an accessible avatar menu and a dedicated personal workspace", async () => {
  ...
  assert.match(menu, /transitionActivationPromise/);
  assert.match(menu, /showAccountActions/);
  assert.doesNotMatch(menu, /正在准备个人账户/);
  assert.doesNotMatch(menu, /微信、QQ 与 Microsoft 登录将在/);
  ...
  assert.doesNotMatch(profile, /检测到受保护的旧学习数据/);
  assert.doesNotMatch(profile, /外部登录正在等待平台配置/);
...
test("implements external OAuth sessions, account linking, private avatars, and user-owned data", async () => {
  ...
  assert.match(auth, /ACCOUNT_ACTIVATION_FAILED/);
  assert.match(auth, /ON CONFLICT\(user_email, language, topic_index\) DO UPDATE SET/);
  assert.match(auth, /mergedTopics\[language\] = Math\.max/);
  assert.match(auth, /DELETE FROM learning_progress WHERE user_email = \?/);
